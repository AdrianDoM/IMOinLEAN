...
import tactic
import .int .mv_polynomial .finset .homogeneous .zmod .coprime .prime .modeq .bezout .multiset

/-!
# IMO 2017 Q6

An ordered pair `(x,y)` of integers is a primitive root if the greatest common divisor of
`x` and `y` is `1`. Given a finite set S of primitive points, prove that there exists a positive
integer `n` and integers `a₀, a₁, ..., aₙ` such that, for each `(x,y)` in `S`, we have:

  `a₀xⁿ + a₁xⁿ⁻¹y + a₂xⁿ⁻²y² + ⋯ + aₙ₋₁xyⁿ⁻¹ + aₙyⁿ = 1.α`
-/

noncomputable theory

open mv_polynomial (hiding X) int (hiding range)
...
attribute [instance] semiring.to_semimodule
...
begin
  ...
  rw [hn1, neg_smul, one_smul] at h,
  rw [←neg_neg p], exact neg h,
end
...
to `±a` at `(-x, -y)`. This shows that it is enough to consider a set `S` where no two primitive
roots are multiples of each other -/
...
begin
  ...
  {	use [T, subset.trans hTR (subset_insert _ _), hTprim, hTmul],
    intros a ha,
    rcases finset.mem_insert.mp ha with rfl | ha, swap,
    {	exact hT a ha },
...
lemma g_degree_ge {s : ℤ × ℤ} (hs : s ∈ S) {n : ℕ} (hn : S.card - 1 ≤ n) :
  { φ : polyℤ // φ.is_homogeneous n ∧ ∀ t ∈ S, eval (to_val t) φ = eval (to_val t) (g S s) } :=
begin
  ...
  intros t ht, rw [eval_mul, eval_pow],
  by_cases hts : t = s,
  {	rw [hts, hψeval, one_pow, one_mul] },
  rw [(g_zero_iff S hSprim hSmul hs ht).mpr hts, mul_zero],
end
omit hSprim hSmul
...
begin
  ...
  simp [← zmod.int_coe_eq_int_coe_iff] at *,
  simp [hx1, hy1],
  convert_to ¬ (↑2 : zmod p) = ↑0, { simp, ring_nf },
...
begin
  ...
  { -- Show that 0 < n
    apply nat.list_lcm_pos, intros n hn,
  ...
  { -- Show that the constructed polynomial is homogeneous
    ...
    intros φ' hφ',
    simp only [ψs, φs, list.pmap, list.mem_pmap, list.mem_map, prod.mk.inj_iff,
      list.mem_erase_dup, list.map, prod.exists] at hφ',
    ...
    convert nat.list_dvd_lcm _ n _,
  ...
  {	rw [← list.forall₂_eq_eq_eq, list.forall₂_iff_zip], split, { simp },
    ...
    have hdvd : (p : ℤ) ^ k ∣ ↑a,
    { rcases hpk with ⟨p, hp, rfl, rfl⟩, norm_cast, apply nat.pow_count_factors_dvd },
    ...
    rcases (homogeneous_one_at_coprime_of_prime_power p k
      (@hprimes ⟨p, k⟩ (by simp [hpk]))).property with ⟨_, _, h⟩,
    change eval (to_val t) (homogeneous_one_at_coprime_of_prime_power p k _).val.fst ≡ 1 [ZMOD ↑p ^ k],
...
def imo2017q6 (S : finset (ℤ × ℤ)) (hS : ∀ t ∈ S, primitive_root t) :
  { φn : polyℤ × ℕ // 0 < φn.2 ∧ φn.1.is_homogeneous φn.2 ∧ 
